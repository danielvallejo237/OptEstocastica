...
{
  /*
  Los índices en donde haremos los cambios de los vecinos, vamos a considerar un cambio como 
  intercambiar un índice del posible vecino al otro, es decir solamente cambiamos una posición del arreglo 
  */
public:
  int indP;
  int indN; //Para checar que el nodo ha sido visitado necesitamos sacarlo de un a lista y ponerlo en otra nueva
  Descriptor(int i, int j)
  {
    indP=i;
    indN=j;
  }
...
class IPairPQ :: public std::priority_queue<iPair,vector<iPair>, greater<iPair> >
{
public:
  bool remove(const int &idx)
  {
    //Removemos algún elemento de la cola de prioridad 
  }
}

class Container
{
  /*La clase contenedor se conforma de un vector de listas de prioridad de parejas de números donde cada pareja corresponde a
  una pareja con el nodo y con el costo obtenido por la matriz de costo real calculada por el algoritmo de Dijkstra*/
public:
  vector<priority_queue<iPair,vector<iPair>,greater<iPair> > > container;
  Container(int nodes,vector<int> used, vector<int> available, const vector<vector<int> > &C)
  {
    //Recibe los vectores de nodos usados, los de nodos disponibles y la correspondiente matriz de costo
    this->container.resize(nodes); 
  }
}
